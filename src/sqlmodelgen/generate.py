...
class ModelGenerator:
    ...
    """
    def __init__(self, output_dir: str, enum_output_dir: str, template_dir: str, preview: bool = False):
    ...
    def _map_type(self, col: Dict[str, Any], enums: Dict[str, List[str]], type_overrides: Dict[str, str] = None) -> str:
    ...
    def _is_one_to_one(self, fk: Dict[str, Any], unique_constraints: List[List[str]]) -> bool:
    ...
    def _is_association_table(self, table: str, columns: List[Dict[str, Any]], fks: List[Dict[str, Any]]) -> bool:
        # Association table: only two FKs, both are PKs, and no other columns
        if len(fks) == 2 and len(columns) == 2:
            pk_cols = [c['name'] for c in columns if not c.get('nullable', True) and c.get('primary_key', False)]
            fk_cols = [fk['constrained_columns'][0] for fk in fks]
            return set(pk_cols) == set(fk_cols)
        return False

    def build_relationship_map(self, all_tables: List[str], all_fks: Dict[str, List[Dict[str, Any]]], all_columns: Dict[str, List[Dict[str, Any]]], all_uniques: Dict[str, List[List[str]]] = None) -> Dict[str, List[Dict[str, Any]]]:
        """
        Build a map of table -> list of relationship dicts, including reverse relationships.
        Supports one-to-one and many-to-many.
        """
        rel_map = {t: [] for t in all_tables}
        all_uniques = all_uniques or {t: [] for t in all_tables}
        # Detect association tables for many-to-many
        association_tables = set()
        for table in all_tables:
            if self._is_association_table(table, all_columns[table], all_fks.get(table, [])):
        ...
        for table, fks in all_fks.items():
            for fk in fks:
                ...
                rel_map[table].append({
                    ...
                    'back_populates': _plural(table) if not is_one_to_one else table,
                ...
                rel_map[fk['referred_table']].append({
                    ...
                    'target_table': table,
                    'back_populates': fk['constrained_columns'][0].replace('_id', ''),
    ...
    def generate_model(self, table: Dict[str, Any], columns: List[Dict[str, Any]], relationships: List[Dict[str, Any]], enums: Dict[str, List[str]], type_overrides: Dict[str, str] = None, rel_map: Optional[Dict[str, List[Dict[str, Any]]]] = None) -> str:
        """
        ...
        for col in columns:
            ...
            is_array = col.get('is_array') or str(col.get('type', '')).upper() == 'ARRAY'
            ...
            if is_uuid:
                ...
                extra_imports.add('from sqlalchemy.dialects.postgresql import UUID as PG_UUID')
                if is_primary:
                ...
                array_type = 'String' if element_type in ('str', 'String') else element_type
                ...
                if py_type == 'Json':
                    ...
                    extra_imports.add('from sqlalchemy.dialects.postgresql import JSONB as PG_JSON')
                if py_type == 'dict':
                ...
                if py_type in enums:
                    ...
                    field_args.append(f'sa_type=SAEnum({py_type}, name="{py_type.lower()}")')
        ...
        if rel_map:
            ...
            for rel in rel_map.get(this_table, []):
                ...
                if rel['is_many_to_many']:
                    ...
                    extra_imports.add('from typing import List')
                    rel_lines.append(f"{rel['field_name']}: {rel_type} = Relationship(back_populates=\"{rel['back_populates']}\", link_model={_camel(rel['association_table'])})")
                elif rel['is_one_to_one']:
                    rel_type = f"None | {target}"
                    rel_lines.append(f"{rel['field_name']}: {rel_type} = Relationship(back_populates=\"{rel['back_populates']}\")")
                elif rel['is_list']:
                    rel_type = f"List[{target}]"
                    extra_imports.add('from typing import List')
                    rel_lines.append(f"{rel['field_name']}: {rel_type} = Relationship(back_populates=\"{rel['back_populates']}\")")
            ...
            for rel in relationships:
                ...
                rel_lines.append(f"{rel_name}: None | {target} = Relationship(back_populates=\"{table['name']}\")")
    ...
    def write_model_file(self, class_name: str, code: str, written_files: Optional[Set[str]] = None):
        """
        Write the generated model code to the output directory.
        """
        os.makedirs(self.output_dir, exist_ok=True)
        path = os.path.abspath(os.path.join(self.output_dir, f"{class_name.lower()}.py"))
    ...
    def write_enum_file(self, enum_name: str, code: str, written_files: Optional[Set[str]] = None):
        """
        Write the generated enum code to the enum output directory.
        """
        os.makedirs(self.enum_output_dir, exist_ok=True)
        path = os.path.abspath(os.path.join(self.enum_output_dir, f"{enum_name.lower()}.py"))
        if written_files is not None:
            written_files.add(path)
        if self.preview:
    ...
    def cleanup_old_files(self, keep_files: Set[str], target_dir: str):
