...
app = typer.Typer(help="SQLModelGen: Generate SQLModel models from a Postgres database.")
console = Console()
...
@app.command()
def generate(
    config: str = typer.Option(None, help="Path to config file. If not provided, will look for sqlmodelgen.yaml/yml/toml in the current directory."),
    ...
    try:
        if config is None:
            ...
            for fname in ["sqlmodelgen.yaml", "sqlmodelgen.yml", "sqlmodelgen.toml"]:
                ...
                console.print("[red]No config file provided and no default config found in the current directory.[/red]")
                sys.exit(1)
        ...
        generator = ModelGenerator(output_dir, enum_output_dir, template_dir, preview=preview)
        tables = [t for t in tables if t not in exclude_tables]
        written_model_files = set()
        written_enum_files = set()
        skipped_models = 0
        skipped_enums = 0
        deleted_files = []
        if preview:
            console.print("[yellow][PREVIEW][/yellow] No files will be written or deleted.")
        if not tables:
            ...
            if os.path.exists(path):
                with open(path, 'r', encoding='utf-8') as f:
                    if f.read() == code:
                        skipped_enums += 1
                        continue
            generator.write_enum_file(enum_name.title().replace('_', ''), code, written_enum_files)
        ...
        # Generate models
        for table in tables:
            columns = [c for c in introspector.get_columns(table) if c['name'] not in exclude_columns]
            relationships = introspector.get_foreign_keys(table)
            code = generator.generate_model({'name': table}, columns, relationships, enums, type_overrides, rel_map=rel_map if relationship_mode == 'full' else None)
            ...
            if os.path.exists(path):
                with open(path, 'r', encoding='utf-8') as f:
                    if f.read() == code:
                        skipped_models += 1
                        continue
            generator.write_model_file(table.title().replace('_', ''), code, written_model_files)
        # Cleanup old files
        if cleanup:
        ...
        console.print(f"[bold]Models written:[/bold] {len(written_model_files)} | [bold]skipped:[/bold] {skipped_models}")
        console.print(f"[bold]Enums written:[/bold] {len(written_enum_files)} | [bold]skipped:[/bold] {skipped_enums}")
        if deleted_files:
...
@app.command()
def inspect(config: str = typer.Option(None, help="Path to config file. If not provided, will look for sqlmodelgen.yaml/yml/toml in the current directory.")):
    ...
    try:
        ...
        for t in tables:
            columns = [c for c in introspector.get_columns(t) if c['name'] not in exclude_columns]
            col_names = ', '.join([c['name'] for c in columns])
            fks = introspector.get_foreign_keys(t)
            fk_str = ', '.join([f"{fk['constrained_columns'][0]}→{fk['referred_table']}.{fk['referred_columns'][0]}" for fk in fks]) if fks else ""
