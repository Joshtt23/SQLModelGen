...
def load_config(path: str) -> Dict[str, Any]:
    ...
    with open(path, "rb" if ext == ".toml" else "r", encoding=None if ext == ".toml" else "utf-8") as f:
        if ext in {".yaml", ".yml"}:
...
def validate_config(cfg: Dict[str, Any]):
    ...
    if 'field_type_overrides' in cfg and not isinstance(cfg['field_type_overrides'], dict):
        raise ConfigError("field_type_overrides must be a dict")
    if 'relationship_mode' in cfg and cfg['relationship_mode'] not in ('minimal', 'full'):
