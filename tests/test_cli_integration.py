...
def test_generate_command(mock_modelgen, mock_introspector, mock_load_config, tmp_path):
    # Create a dummy config file
    config_path = tmp_path / 'fake.yaml'
    config_path.write_text('database_url: postgresql://user:pass@localhost/db\noutput_dir: models\nenum_output_path: enums\n')
    ...
    mock_introspect_instance.get_enums.return_value = {'UserStatus': ['active', 'inactive']}
    ...
    mock_modelgen_instance.generate_enum.assert_called_with('Userstatus', ['active', 'inactive'])
...
@patch('sqlmodelgen.cli.load_config')
@patch('sqlmodelgen.cli.Introspector')
def test_inspect_command(mock_introspector, mock_load_config):
    mock_load_config.return_value = {'database_url': 'postgresql://user:pass@localhost/db'}
    mock_introspect_instance = MagicMock()
    mock_introspect_instance.get_tables.return_value = ['user']
    mock_introspect_instance.get_enums.return_value = {'UserStatus': ['active', 'inactive']}
    mock_introspect_instance.get_columns.return_value = [
